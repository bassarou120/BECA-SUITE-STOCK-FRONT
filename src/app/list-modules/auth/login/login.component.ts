...
export class LoginComponent implements OnInit, OnDestroy {
  ...
  loginStep: 'LOGIN' | 'OTP' = 'LOGIN';
  otpUserId: string | null = null;
  otp = '';
  ...
  ngOnInit() {
    ...
    console.log(user.role.libelle_role);
  ...
  verifyOtp() {
    ...
    this.authservice.verifyOtp(payload).subscribe(
      (res: any) => {
        ...
        if (res.success === true) {
          ...
          setTimeout(() => {
            ...
            console.log('USER:', localStorage.getItem('userDataString'));
      ...
      (error) => {
        this.showloader = false;
        this.otpError = error.error?.message || 'OTP incorrect ou expiré';
      }
    );
  }
