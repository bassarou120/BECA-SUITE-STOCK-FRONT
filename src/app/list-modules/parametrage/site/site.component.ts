...
import {ExportsService, routes, banqueService, getSite, getBureau} from 'src/app/core/core.index';
...
export class SiteComponent implements OnInit {
  ...
  dataSource!: MatTableDataSource<getSite>;
  ...
  constructor(private formBuilder: FormBuilder,public router: Router, private data: SiteService, private exp: ExportsService) {}
...
getDeleteForm(row: any){
  ...
  private getTableData(): void {
    ...
    this.data.getAll().subscribe((res: any) => {
      ...
      res.data.data.map((res: getSite, index: number) => {
        const serialNumber = index + 1;
        if (index >= this.skip && serialNumber <= this.limit) {
          res.id;// = serialNumber;
          this.lstSite.push(res);
          this.serialNumberArray.push(serialNumber);
        }
      });
      this.dataSource = new MatTableDataSource<getSite>(this.lstSite);
