...
@Component({
  ...
  styleUrls: ['./retraite.component.scss']
})
export class RetraiteComponent implements OnInit {
 ...
 exportToPDF() {
  ...
  $('#spinner_pdf').removeClass('d-none');
    this.exp.exportDepartEmploye("retraite").subscribe(
      (response: any) => {
  ...
  private getTableData(): void {
    ...
    this.data.getAllDepartEmploye().subscribe((res: any) => {
      ...
      res.data.map((res: getDepartEmploye, index: number) => {
        ...
        if (index >= this.skip && serialNumber <= this.limit) {
          if(res.typeDepart.trim().toLowerCase() == "retraite") {
    ...
    this.data.getAllTypeDepart().subscribe((res: any) => {
      res.data.data.map((res: getTypeDepart, index: number) => {
        const serialNumber = index + 1;
        if (index >= this.skip && serialNumber <= this.limit) {
          if(res.lib.trim().toLowerCase() == "retraite") {
